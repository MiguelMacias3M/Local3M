...
try {
    ...
    if ($action === 'reporte_dia' || $action === 'reporte_rango') {
        ...
        foreach ($movs as $m) {
            ...
            // 2. Sumar Gastos (SOLO SI NO ES RETIRO NI CIERRE)
    ...
    if ($action === 'cerrar_caja') {
        ...
        // CALCULO SISTEMA (MODIFICADO: NO RESTA RETIROS NI CIERRES)
        ...
        $totalIng = 0; 
        $totalRet = 0; // Se mantendrá en 0 según tu petición
        
        foreach ($movs as $m) {
            $totalIng += (float)$m['ingreso'];
            
            // ANTES: Aquí se restaba si era RETIRO o CIERRE
            // AHORA: Lo comentamos para que NO reste nada.
            /*
            $t = strtoupper($m['tipo']);
        ...
        // Saldo Esperado = Inicial + Ingresos (Sin restar nada)
        $saldoSistema = (float)$caja['saldo_inicial'] + $totalIng; // - $totalRet (eliminado)
        ...
        if ($retirar === 'SI' && $montoReal > 0) {
            $sqlRet = "INSERT INTO caja_movimientos (id_transaccion, tipo, descripcion, monto_unitario, egreso, usuario, fecha, categoria, origen) VALUES (?, 'CIERRE', 'Retiro por Cierre (Auto)', ?, ?, ?, ?, 'Cierre', 'CAJA')";
...
function obtenerEstadoCaja($conn) {
    try {
        ...
        if ($caja) {
            ...
            $totI = 0; 
            $totR = 0; // Se mantendrá en 0
            foreach($movs as $m) {
                $totI += (float)$m['ingreso'];
                
                // ANTES: Aquí se restaba
                // AHORA: Comentado para que NO reste del monto actual
                /*
                $t = strtoupper($m['tipo']);
                if($t === 'RETIRO' || $t === 'CIERRE') {
                    $totR += (float)$m['egreso'];
            ...
            // Saldo Actual = Inicial + Ingresos (Sin restar retiros)
            $saldoActual = (float)$caja['saldo_inicial'] + $totI; // - $totR (eliminado)

            return ['estado'=>'ABIERTA', 'usuario'=>$caja['usuario_apertura'], 'monto_actual'=>$saldoActual, 'inicio'=>$caja['fecha_apertura'], 'id'=>$caja['id']];
        }
