...
try {
    ...
    if ($action === 'reporte_dia' || $action === 'reporte_rango') {
        ...
        foreach ($movs as $m) {
            ...
            // CORRECCIÓN: No sumar a gastos si es RETIRO o CIERRE
            // Estos son movimientos de flujo de efectivo, no gastos operativos
    ...
    if ($action === 'cerrar_caja') {
        ...
        // CALCULO SISTEMA: Inicial + Ingresos - (Retiros + Cierres previos si hubiera)
    ...
    try {
        ...
        if ($caja) {
            ...
            foreach($movs as $m) {
                ...
                // Restar RETIROS y CIERRES del efectivo actual
                $t = strtoupper($m['tipo']);
                if($t === 'RETIRO' || $t === 'CIERRE') {
                    $totR += (float)$m['egreso'];
                }
